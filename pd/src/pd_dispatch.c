 ...
 *
 * Every inbound command — plaintext or Secure Channel (SCS_15/17) — funnels
 * through osdp_pd_internal_dispatch once its payload is in the clear. This
 * file owns what a command *means*:
 *
 ...
 *
 * Before this existed the whole block above was written out once per channel,
 * and the copies had already drifted — one of them intercepted neither
 * osdp_COMSET nor osdp_FILETRANSFER, so on that path a COMSET fell through to
 * the application handler and the PD never adopted the new address, and a
 * FILETRANSFER never reached the registered receiver. Keeping one copy is what
 * stops that recurring as more library-handled commands land. */
...
/* Apply an osdp_KEYSET the application has just agreed to ACK. The channel is
 * taken because a future Secure Channel version rotating a different key size
 * would need a different variant here; today every channel rotates the
 * AES-128 SCBK. A malformed or unusable key record downgrades the wire reply
 * from ACK to NAK 0x09 ("unable to process command record") and leaves the
...
{
    (void)channel;
    return osdp_pd_internal_apply_keyset(pd, payload, payload_len) == OSDP_OK;
}
