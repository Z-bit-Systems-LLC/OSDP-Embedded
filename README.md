...
- **Secure Channel** (AES-128) — the full SCS_11..18 lifecycle. The consumer
  supplies AES and RNG through a small HAL; the library vendors no crypto.
...
- **Verified against independent implementations** — byte-level capture replay
  ...
  [OSDP.Net](https://github.com/Z-bit-Systems-LLC/OSDP.Net).
...
| **SCS_11..18** | The secure-channel frame types: 11–14 carry the handshake, 15/16 authenticate empty messages, 17/18 authenticate and encrypt data-bearing ones. |
...
The library reaches the outside world through two small callback vtables you
implement:
...
| `osdp_sc_crypto_t` | `aes128_ecb_encrypt`, `aes128_ecb_decrypt`, `rand_bytes` | mbedTLS, wolfCrypt, a hardware AES block, `BCryptGenRandom`, `/dev/urandom` |

An application that never binds a crypto vtable gets a plaintext-only build, with
no AES or RNG code linked in.
...
Both mocks accept `--sc=scbkd` or `--sc=scbk:HEX32` for Secure Channel. Run any tool
