...
impl DemoCrypto {
    fn new(seed: u64) -> Self { Self { rng_state: seed } }
}
...
struct ReplyCapture { inner: Rc<RefCell<Captured>> }
...
struct ScEventCapture { inner: Rc<RefCell<Captured>> }
impl ScEventHandler for ScEventCapture {
    fn on_sc_event(&mut self, e: &ScEvent) {
        self.inner.borrow_mut().sc_events.push((e.pd_address, e.kind));
    }
}
...
fn main() {
    ...
    acu.set_sc_event_handler(ScEventCapture { inner: Rc::clone(&captured) });
    ...
    assert!(acu.is_pd_sc_established(PD_ADDRESS),   "ACU never reached established");

    {
        let cap = captured.borrow();
        assert_eq!(cap.sc_events.len(), 1, "expected exactly one ESTABLISHED event");
        ...
        assert!(cap.replies.is_empty(), "handshake leaked into reply_handler");
    }

    println!("handshake: PD and ACU both report ESTABLISHED");

    // ---- Operational POLL → ACK (SCS_15/16) ----
    acu.send_command(PD_ADDRESS, OSDP_CMD_POLL, &[]).expect("send POLL");
    cycle(&mut pd, &mut acu, 4);

    // ---- Operational ID → PDID (SCS_17/18, encrypted payloads) ----
    let id_request: [u8; 1] = [0x00];
    acu.send_command(PD_ADDRESS, OSDP_CMD_ID, &id_request).expect("send ID");
    cycle(&mut pd, &mut acu, 4);
